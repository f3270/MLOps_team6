...
# Configura MLflow para usar el servidor local en http://127.0.0.1:5000
...
def run_pipeline(data_path, model_output_path):
    ...
    with mlflow.start_run(run_name="Oscar's Bone Marrow Experiment"):
        ...
        # Guardar un fragmento de los datos preprocesados como artefactos
        ...
        # Registrar el modelo en MLflow como artefacto
        mlflow.log_artifact(model_output_path)

        # Registrar el modelo en el Model Registry
        model_name = "Bone_Marrow_Model"
        model_uri = f"runs:/{mlflow.active_run().info.run_id}/{model_output_path}"
        mlflow.register_model(model_uri, model_name)

        print(f"Modelo registrado como '{model_name}' en el Registro de Modelos.")

        # Evaluar el modelo y loggear las métricas
